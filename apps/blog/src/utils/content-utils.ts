...
export function getPostAssetBasePath(
	post: Pick<CollectionEntry<"posts">, "filePath" | "id">,
): string {
	const filePath = post.filePath?.replace(/\\/g, "/");
	if (filePath) {
		const srcContentIndex = filePath.indexOf("src/content/posts/");
		if (srcContentIndex >= 0) {
			return getDir(filePath.slice(srcContentIndex + "src/".length));
		}

		const contentIndex = filePath.indexOf("content/posts/");
		if (contentIndex >= 0) {
			return getDir(filePath.slice(contentIndex));
		}
	}

	const idDir = getDir(post.id);
	return idDir === "/" ? "content/posts/" : `content/posts/${idDir}`;
}
